<div>
    <section class="relative px-4 md:px-8 py-16">
        <div class="mx-auto container">
            ...
            {{-- LOADING INDICATOR --}}
            {{-- Shows only when "category" property is updating --}}
            <div wire:loading wire:target="category" class="z-20 absolute inset-0 flex justify-center items-start bg-white/60 backdrop-blur-[2px] pt-40 transition-all duration-300">
                <div class="flex items-center gap-3 bg-black shadow-2xl px-6 py-3 rounded-full text-white">
                    <svg class="w-5 h-5 text-white animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
                    <span class="font-bold text-xs uppercase tracking-widest">Loading Products...</span>
                </div>
            </div>

            {{-- GRID --}}
            {{-- FIX: wire:key="{{ $category }}" forces Alpine to re-init when category changes --}}
            <div x-data="shopComponent(@js($products))" 
                 wire:key="{{ $category }}"
                        ...
                        <template x-for="product in products" :key="product.id">
                            <div class="group flex flex-col bg-white shadow-sm hover:shadow-lg p-2 transition-shadow duration-300">
                                ...
                                <div class="relative bg-gray-100 aspect-[3/4] overflow-hidden cursor-pointer">
                                    <img :src="product.images[0]" class="w-full h-full object-cover group-hover:scale-105 transition duration-700"
                                        @click="openImageModal(product, 0)">

                                    <button @click="openInfoModal(product)" class="top-3 right-3 z-10 absolute bg-white/90 hover:bg-black shadow-sm backdrop-blur p-2 rounded-full text-black hover:text-white transition">
                                ...
                                <div class="flex justify-between items-start mt-3">
                                    <div>
                                        <h3 x-text="product.name" class="font-medium text-gray-900 text-sm uppercase tracking-wide"></h3>
                ...
                <div x-show="modals.info.open"
                    ...
                    <div class="relative bg-white shadow-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto transition-all transform"
                        ...
                        <button @click="modals.info.open = false" class="top-4 right-4 absolute text-gray-400 hover:text-black">
                        ...
                        <div class="space-y-2 mb-6 text-gray-600 text-sm">
                            <p x-show="modals.info.data?.details.description" class="mb-4 italic" x-text="modals.info.data?.details.description"></p>
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <select required x-model="selectedSize" class="block mt-1 py-2 pr-10 pl-3 border-gray-300 focus:border-indigo-500 rounded-md focus:outline-none focus:ring-indigo-500 w-full sm:text-sm text-base">
                            ...
                            <div>
                                ...
                                <div class="flex items-center mt-1 border border-gray-300 rounded w-32">
                                    <button @click="selectedQty > 1 ? selectedQty-- : null" class="bg-gray-100 hover:bg-gray-200 px-3 py-1">-</button>
                                    <input type="number" x-model="selectedQty" class="p-1 border-none focus:ring-0 w-full text-center" min="1">
                                    <button @click="selectedQty++" class="bg-gray-100 hover:bg-gray-200 px-3 py-1">+</button>
                                </div>
                            </div>
                        </div>
                ...
                {{-- Image Modal --}}
                <div x-show="modals.image.open" x-transition.opacity style="display: none;" class="z-50 fixed inset-0 flex justify-center items-center bg-black/90 backdrop-blur-sm p-4" x-cloak>
                      <button @click="modals.image.open = false" class="top-5 right-5 absolute text-white hover:text-gray-300">
                    ...
                    <div class="relative flex flex-col items-center w-full max-w-4xl max-h-full" @click.outside="modals.image.open = false">
